...
do
						...
						if stomp then
							...
							if arrest and isCop() then
								...
								if os.clock()-lastArrest > 0.1 then
									...
									lastArrest = os.clock()
									--[[
									local cac = killPlayer.Parent:FindFirstChild(killPlayer.Name)
									task.delay(2,function()
										if cac == killPlayer and cuffs.Parent == char then
											cuffs:Activate()
											lastArrest = os.clock()
										end
									end)
	...
	KillAll:addButton("Arrest All",function()
		...
		for i,v in pairs(targets) do
			...
			local df = v:FindFirstChild("DataFolder")
			updateCrew()
			if v == player or not (df and ls) or df.Officer.Value ~= 0 or v.leaderstats.Wanted.Value <= 0 then --or (Crew == otherCrew and Crew ~= 0)
				-- exclude them
			else
				table.insert(_t,v)
			end
		...
		targets = _t -- shallow copy

		if #targets <= 0 then
			Artemis:Notify("Arrest Error","Nobody is in the server!")
		end
		...
		killAllHook = HB:Connect(function()
			...
			local s,r = pcall(function()
				...
				local now = os.clock()
				local noTarget = not (target and target.Character)
				if noTarget and now-arrestPause >= 24 then
					pcall(function()
						killAllPlayerHook:Disconnect()
					end)
				...
				elseif noTarget then
					tp(AfkSpots["Admin Base"])
				end
				killPlayer = target
			end)

			if not s then warn("Error encountered during arrest all loop: "..r) end
