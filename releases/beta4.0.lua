...
local beta = true -- change to false when exiting beta!
...
local CrewValue = Information:WaitForChild("Crew")
...
do
	...
	function LoadFiles() -- looks through all files to make sure that every file is correct (not edited) and fills in any missing/corrupted required files
		...
		local order = {"Date","Time","Formatted Time","Beta"}

		local function removeZero(number)
			number = tostring(number)
			if string.sub(number,1,1) == "0" then
				number = string.sub(number,1,-1)
			end
...
local function logAction(text,toggle)
	local textAdd = nil
...
player.Idled:Connect(function()
	...
	logAction("Anti-AFK prevented player from being disconnected")
end)
...
logAction("Artemis fully loaded\n")
