...
local function mobileCharacter(char)
	...
	if validCharacter(character) then
		local ko = character:WaitForChild("BodyEffects"):FindFirstChild("K.O")
...
local ToggleUseDisplayNameForTeleport1 = TeleportToPlayer:addToggle("Use Display Name",useDisplayName1,function(newValue)
...
local KillPlayerInput = KillPlayer:addSuggestionBox("Chosen Player","",function(text,focusLost)
	if focusLost then
		print(text)
...
do	-- load audios
	...
	local playAudio = Audio:addButton("Play Audio",function()
	...
	local function onAdded()
		...
		alert.BackgroundColor3 = Color3.fromRGB(138, 138, 138)
		alert.BackgroundTransparency = 0.5
		alert.BorderColor3 = Color3.fromRGB(27, 42, 53)
		alert.TextColor3 = Color3.new(0,0,0)
		alert.BorderSizePixel = 1
		alert.Font = Enum.Font.Gotham
		alert.Position = UDim2.fromOffset(0,40)
		alert.Size = UDim2.fromOffset(300,50)
		alert.TextScaled = true
		alert.Text = "Please use Artemis to play audios!"
	
		alert.Parent = boomboxFrame
		]]--
	end
...
-- Cache
local ClearAudioCache = Cache:addButton("Clear Audio Cache",function()
	audioInfo = {}
end)
